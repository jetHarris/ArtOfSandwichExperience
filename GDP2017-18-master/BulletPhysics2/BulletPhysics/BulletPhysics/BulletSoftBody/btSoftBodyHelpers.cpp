...
int								nattrb=0;
int								hasbounds=0;
int result = sscanf_s(node,"%d %d %d %d",&nnode,&ndims,&nattrb,&hasbounds);
result = sscanf_s(node,"%d %d %d %d",&nnode,&ndims,&nattrb,&hasbounds);
node += nextLine(node);
...
for(int i=0;i<pos.size();++i)
	...
	//int			bound=0;
	float	x,y,z;
	sscanf_s(node,"%d %f %f %f",&index,&x,&y,&z);
...
if(ele&&ele[0])
	...
	int								ncorner=0;
	int								neattrb=0;
	sscanf_s(ele,"%d %d %d",&ntetra,&ncorner,&neattrb);
	ele += nextLine(ele);
	...
	for(int i=0;i<ntetra;++i)
		...
		sscanf_s(ele,"%d %d %d %d %d",&index,&ni[0],&ni[1],&ni[2],&ni[3]);
		ele+=nextLine(ele);
