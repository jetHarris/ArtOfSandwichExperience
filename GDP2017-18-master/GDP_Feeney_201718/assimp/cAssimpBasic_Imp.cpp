	...
	std::string &error)
{
	...
	//const aiScene* scene = importer.ReadFile(this->m_basePath + filename,
	...
	//filename = cCurrentPath + filename;

	const aiScene* scene = mImporter.ReadFile(filename.c_str(), Flags);
	...
	if ( scene->HasMeshes() )			// or ( scene->mNumMeshes > 0 )
	{
		...
		theMesh.pVertices = new sVertex_xyz_rgba_n_uv2[theMesh.numberOfVertices];
